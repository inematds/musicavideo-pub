...
export function musicas(acervo) {
  ...
  for (const x of acervo.musicavideo) {
    for (const f of x.faixas || []) {
      ...
      vistos.add(id);
      // `thumb` é a capa reduzida (~30 KB de JPEG contra 1,2 MB de PNG): é ela
      // que o card da grade pede. A capa cheia continua vindo em `capa`, para
      // o detalhe, o poster do vídeo e o clique em tamanho real.
      fora.push({
        id,
        producao: x,
        faixa: { ...f, capa: f.capa || x.capa, thumb: f.thumb || x.thumb || f.capa || x.capa },
      });
    }
  }
  return fora;
